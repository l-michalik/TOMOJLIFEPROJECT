...
This project contains a basic `Supervisor` API backed by `deepagents`. It accepts a normalized DevOps task request, validates completeness before planning, and returns either a clarification request or a structured planning contract with workflow state, plan steps, confidence, and risk metadata.
